...
function load() {
  g_s= "true";
  document.getElementById("b2").innerHTML= "game is loading";
}
...
console.log("noseX="+noseX+"noseY="+noseY);
  ...
  if(gameConfig.status==='start'){
    ...
    textSize(40);
    textAlign(CENTER);
    text("Move your nose to play", gameConfig.screenX/2, gameConfig.screenY/2);
    textSize(40);
...
function changeGameStatud(character){
  if( g_s="true" && gameConfig.status==="start" && noseX!="") {
    world_start.play();
